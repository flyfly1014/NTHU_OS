...
bool FileSystem::Remove(char *name, bool is_recursive)
{
    ...
    if (is_recursive)
    {
        DirectoryEntry *table = directory->get_table();
        int tableSize = directory->get_tablesize();
        for (int i = 0; i < tableSize; i++)
        {
            if (table[i].inUse)
            {
                ...
                if (table[i].isDir)
                {
                    Remove(new_path, true);
                }
                else
                {
                    Remove(new_path, false);
                }
            }
        }
    }
...
// 	List all the files in the file system directory and subdirectory.
//----------------------------------------------------------------------
